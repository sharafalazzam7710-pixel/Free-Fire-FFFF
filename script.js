...
let settings = normalizeSettings(read(KEYS.settings, {}));
...
function normalizeSettings(value = {}) {
  ...
  const savedPreference = value?.registrationAfterFinish || value?.registration || DEFAULT_SETTINGS.registrationAfterFinish;
  merged.registrationAfterFinish = savedPreference === 'closed' ? 'closed' : 'open';
  if (merged.started && !merged.finished) merged.registration = 'closed';
  if (merged.finished) merged.registration = merged.registrationAfterFinish;
  return merged;
}
...
function syncRegistrationState() {
  ...
  const tournamentActive = isStarted && !isFinished;
  // أثناء البطولة يكون التسجيل مقفولًا دائمًا، وبعد انتهائها نعود
  // إلى الاختيار الذي حدده المشرف من قائمة حالة التسجيل.
  $('registrationAlready').classList.toggle('hidden', !ownTeam);
  $('registrationPanel').classList.toggle('hidden', Boolean(ownTeam) || tournamentActive || !isOpen);
  $('registrationClosed').classList.toggle('hidden', Boolean(ownTeam) || tournamentActive || isOpen);
  $('tournamentStarted').classList.toggle('hidden', Boolean(ownTeam) || !tournamentActive);
...
function fillSettingsForm() {
  ...
  // أثناء البطولة نعرض تفضيل المشرف لما سيحدث بعد الانتهاء، لأن
  // التسجيل الفعلي يكون مقفولًا تلقائيًا.
  $('settingsRegistration').value = settings.started && !settings.finished
    ? settings.registrationAfterFinish
...
$('settingsForm').addEventListener('submit', (event) => {
  ...
  const willFinish = $('settingsFinished').checked;
  const startDate = $('settingsStartDate').value;
  const startTime = $('settingsStartTime').value;
  const selectedRegistration = $('settingsRegistration').value;
  ...
  settings = {
    ...
    // بدء البطولة يقفل التسجيل فورًا مهما كان الاختيار السابق.
    // الاختيار يبقى محفوظًا ليُطبّق مباشرة عند إنهاء البطولة.
    registration: willFinish ? selectedRegistration : (willStart ? 'closed' : selectedRegistration),
    registrationAfterFinish: selectedRegistration,
    started: willStart,
    finished: willFinish,
    startDate,
    startTime,
  ...
  renderAll();
  showToast(startingNow ? 'بدأت البطولة وأُغلق التسجيل تلقائيًا' : 'تم حفظ إعدادات البطولة');
});
